...
import { Position, Velocity, Alien, Rotation, Asteroid, Player } from '../components/index';
...
world.system('AlienAI')
  ...
  .each([Position, Velocity, Alien, Rotation], (e, [pos, vel, alien, rot]) => {
    ...
    if (closestAst !== null && minAstDist < ENTITY_CONFIG.ALIEN.ASTEROID_AVOID_DIST) {
      ...
      if (Math.random() < 0.5) {
        ...
        createBullet(pos.x, pos.y, angleToAst, '#ffaa00', 'alien', CAT_ENEMY_BULLET, CAT_PLAYER | CAT_ASTEROID);
    ...
    if (alien.shootCooldown <= 0) {
      ...
      for (const pe of playerQuery.entities) {
        ...
        if (d < minDist) { minDist = d; closestPlayer = pe; }
      }
      if (closestPlayer !== null && minDist < ENTITY_CONFIG.ALIEN.TARGET_DIST_MAX) {
        const pp = closestPlayer.get(Position)!;
        createBullet(pos.x, pos.y, Math.atan2(pp.y - pos.y, pp.x - pos.x), '#ffaa00', 'alien', CAT_ENEMY_BULLET, CAT_PLAYER | CAT_ASTEROID);
        alien.shootCooldown = ENTITY_CONFIG.ALIEN.SHOOT_COOLDOWN_BASE + Math.random() * ENTITY_CONFIG.ALIEN.SHOOT_COOLDOWN_RANGE;
      } else {
